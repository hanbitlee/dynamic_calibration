...
import SimulationModels.MacroEconABM.postProcessSimulationResult as MacroEconModelPost
...
class DynamicCalibration():
    ...
    def __init__(self, hyperParameters, validationObservation):
        ...
        elif self.hyperParameters.modelName == 'MacroEconABM':
            self.post = MacroEconModelPost.postProcess(hyperParameters)
    ...
    def iterateCalibration(self, itrCalibration, currentParameters, simResult, simResultCov):
        if itrCalibration % (self.hyperParameters.dynIters + self.hyperParameters.hetIters) < self.hyperParameters.dynIters:
            ...
            if not os.path.exists('Regimes.csv'):
                with open('Regimes.csv', 'w', encoding='UTF8', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([itrCalibration, regimePerCandidate, mergedRegimesSet])
                    f.close()
            else:
                with open('Regimes.csv', 'a', encoding='UTF8', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([itrCalibration, regimePerCandidate, mergedRegimesSet])
                    f.close()

            # Plot Regime Detection
            #self.plts.plotRegimeDetection(itrCalibration, regimePerCandidate, mergedRegimesSet)
                ...
                for time in range(self.hyperParameters.numTimeStep):
                    for ss in range(len(self.trueResult[time])):
                        if simResult[candidate][time][ss] == 0.:
                            ...
                            val = norm.pdf(self.trueResult[time][ss], simResult[candidate][time][ss], simResultCov[candidate][time][ss][ss] + 0.01 * simResult[candidate][time][ss])
                            weights[candidate][time] += val
    ...
    def obtainRegime(self, differences, itrCalibration):
        ...
        for candidate in range(self.hyperParameters.numCandidate):
            if self.hyperParameters.regimeDetectionAlgorithm == 'HMM':
                ...
                print("Pearson correlation coefficient of regime sequences")
                print(np.corrcoef(regimePerCandidate[0], regimePerCandidate[1]))
                print(np.corrcoef(regimePerCandidate[0], regimePerCandidate[2]))
                print(np.corrcoef(regimePerCandidate[1], regimePerCandidate[2]))

            elif self.hyperParameters.regimeDetectionAlgorithm == 'HSMM':
                regimePerCandidate[candidate] = LearningModels.CalibrationLearningModel().useHSMM(
                    self.hyperParameters.numTimeStep,
                    self.normalize(differences[candidate]), self.trueResult, self.hyperParameters.dir, candidate,
                    itrCalibration, self.hyperParameters.HMMClusters, self.hyperParameters.numOutputDim)
