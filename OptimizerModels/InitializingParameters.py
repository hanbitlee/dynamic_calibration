...
class initializingParameters():
    ...
    def initializeParameters(self):
        ...
        if self.hyperParameters.modelName == 'RealEstateMarketABM':
            ...
            file = open(os.getcwd() + '/SimulationModels/' + self.hyperParameters.modelName + '/InputParameters/DynamicParameters.csv','r', encoding='latin-1')
            lines = file.readlines()
            file.close()
            flag = 0 #YK_check : no need to use 'file'

            numEstParams = 2
            paramMin = [0.02, 0.3] #YK_check
            paramMax = [0.06, 0.7] #YK_check
            dynamicParameter = np.zeros((numEstParams, self.hyperParameters.numCandidate, self.hyperParameters.numTimeStep))

            for estParam in range(numEstParams):
                for candidate in range(self.hyperParameters.numCandidate):
                    #for time in range(self.hyperParameters.numTimeStep): #YK_check : same initial parameters for all timesteps?
                        dynamicParameter[estParam][candidate] = np.random.uniform(low=paramMin[estParam], high=paramMax[estParam])
        ...
        dynParamMin = np.transpose(
            np.tile(paramMin[:dynamicParameter.shape[0]], int(dynamicParameter.shape[1] * dynamicParameter.shape[2])) \
            .reshape(-1, dynamicParameter.shape[0])).reshape(dynamicParameter.shape[0],
                                                             self.hyperParameters.numCandidate, -1)
        dynParamMax = np.transpose(
            np.tile(paramMax[:dynamicParameter.shape[0]], int(dynamicParameter.shape[1] * dynamicParameter.shape[2])) \
            .reshape(-1, dynamicParameter.shape[0])).reshape(dynamicParameter.shape[0],
                                                             self.hyperParameters.numCandidate, -1)
